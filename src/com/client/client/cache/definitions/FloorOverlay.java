...
import com.client.client.FileOperations;
...
public class FloorOverlay {
    ...
    public static void unpackConfig(CacheArchive streamLoader) {
    	 ByteBuffer buffer = ByteBuffer.wrap(FileOperations.readFile(signlink.findcachedir() + "overlay.dat"));
        ...
        for (int i = 0; i < count ; i++) {
            ...
            overlayFloor[i].readValues(buffer);
            if(i == 113)
	...
	private void readValues(ByteBuffer byteBuffer) {
		for (;;) {
			...
			if (attributeId == 0) {
				...
				System.err.println("[FloorOverlay] Missing AttributeId: " + attributeId);
			}
		}
	}
