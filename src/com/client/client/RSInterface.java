...
public class RSInterface {
	...
	public static void MBoxInterface(TextDrawingArea[] tda) {
		String dir = "Interfaces/Vote/sprite";
		
        RSInterface widget = addInterface(21800);
		
		addSprite(21801, 0, dir);
		
		//addRealHoverButton(21802, dir, 6, 7, 21, 21, "Close", -1, 21803, 1);
		
		addText(21805, "Vote box", 0xff9933, true, true, -1, tda, 2);
		
		//addRealHoverButton(21811, dir, 3, 4, 138, 20, "Spin it!", -1, 21812, 1);
		
		addText(21814, "Spin it!", 0xff9933, true, true, -1, tda, 2);


		addText(21815, "Possible loot:", 0xff9933, true, true, -1, tda, 2);

		addText(21816, "Latest loot:", 0xff9933, true, true, -1, tda, 2);

        RSInterface rewards = addTabInterface(21806);
        rewards.width = 462;
        rewards.height = 100;
        addVote(21807, 12);
        addSprite(21808, 1, dir);
		addSprite(21810, 2, dir);
		
		RSInterface possibleRewards = addTabInterface(21820);
		possibleRewards.width = 252;
		possibleRewards.height = 135;
		possibleRewards.scrollMax = 462;
        addRewards(21822);
        
		setChildren(1, possibleRewards);
		setBounds(21822, 5, 4, 0, possibleRewards); //Rewards

		RSInterface lastRewards = addTabInterface(21830);
		lastRewards.width = 127;
		lastRewards.height = 135;
		lastRewards.scrollMax = 168;
		addLastRewards(21831);
        
		setChildren(1, lastRewards);
		setBounds(21831, 5, 4, 0, lastRewards); //Rewards
        
		setChildren(3, rewards);
		setBounds(21808, 0, 0, 0, rewards); //Background
		setBounds(21807, 5, 5, 1, rewards); //Rewards container
		setBounds(21810, 462 / 2 - 10 / 2, 0, 2, rewards); //Arrow
		
		setChildren(8, widget);
		setBounds(21801, 10, 17, 0, widget); //Background
		
		//setBounds(21802, 474, 24, 1, widget); //Close

		setBounds(21805, 262, 27, 1, widget); //Title
		
		setBounds(21806, 26, 100, 2, widget); //Rewards layer
		
		//setBounds(21811, 187, 66, 4, widget); //Spin it!

		setBounds(21814, 256, 67, 3, widget); //Text of button
		
		setBounds(21820, 26, 165, 4, widget); //Possible rewards

		setBounds(21815, 65, 145, 5, widget); //Possible rewards text
		
		setBounds(21830, 343, 165, 6, widget); //Latest rewards

		setBounds(21816, 375, 145, 7, widget); //Text on the right
	}
	
	public static void addLastRewards(int i) {
	...
	public static void addRewards(int i) {
		...
		rsinterface.actions = new String[5];
		rsinterface.spritesX = new int[20];
		rsinterface.invStackSizes = new int[66];
		rsinterface.inv = new int[66];
		rsinterface.spritesY = new int[20];
		rsinterface.children = new int[0];
		rsinterface.childX = new int[0];
		rsinterface.childY = new int[0];
	...
	public static void addVote(int i, int width) {
		RSInterface rsinterface = interfaceCache[i] = new RSInterface();
		rsinterface.actions = new String[5];
		rsinterface.spritesX = new int[20];
		rsinterface.invStackSizes = new int[30];
	   ...
	   /**
		 * lms for now
		 */
		public static void LmsPanel(TextDrawingArea[] tda) {
			RSInterface RSinterface = addInterface(22119);
			addText(22120, "", 0x999999, false, true, 52, tda, 2);
			addText(22121, "", 0x999999, false, true, 52, tda, 2);
			addText(22122, "", 0x999999, false, true, 52, tda, 2);
			addText(22123, "", 0x999999, false, true, 52, tda, 2);
			int last = 4;
			RSinterface.children = new int[last];
			RSinterface.childX = new int[last];
			RSinterface.childY = new int[last];
			setBounds(22120, 15, 12, 0, RSinterface);
			setBounds(22121, 15, 30, 1, RSinterface);
			setBounds(22122, 15, 48, 2, RSinterface);
			setBounds(22123, 15, 66, 3, RSinterface);
		}
		
	private static void dropTableCheckerInterface(TextDrawingArea[] rsFont) {
		final RSInterface main_widget = addTabInterface(37600);
		setChildren(23, main_widget);

		addSpriteLoader(28552, 1054);
		setBounds(28552, 27, 45, 4, main_widget);

		addSpriteLoader(37601, 1053);
		...
		addHoverButtonWSpriteLoader(37603, 1015, 21, 21, "Close", -1, 37604, 3);
		addHoveredImageWSpriteLoader(37604, 1016, 21, 21, 2);
		...
		addText(37914, "0.2%\\n@blu@1/500", 0xFFFFFF, true, true, -1, 1);
		setBounds(37914, xPosition + 12, 55, 20, main_widget);
		xPosition += 39;

		addText(37926, "0.1%\\n@blu@1/1000", 0xFFFFFF, true, true, -1, 1);
		...
		clickable_text_widget.scrollMax = 2500;
		...
		addToItemGroup(37915, 1, 60, 8, 8, false, null, null, null);
		setBounds(37915, xPosition, 7, 70, item_widget);
		xPosition += 39;

		addToItemGroup(37916, 1, 60, 8, 8, false, null, null, null);
		setBounds(37916, xPosition, 7, 71, item_widget);
		xPosition += 39;

		addToItemGroup(37917, 1, 60, 8, 8, false, null, null, null);
		setBounds(37917, xPosition, 7, 72, item_widget);
		xPosition += 39;

		addToItemGroup(37918, 1, 60, 8, 8, false, null, null, null);
		setBounds(37918, xPosition, 7, 73, item_widget);
		xPosition += 39;

		addToItemGroup(37919, 1, 60, 8, 8, false, null, null, null);
		setBounds(37919, xPosition, 7, 74, item_widget);
		xPosition += 39;

		addToItemGroup(37920, 1, 60, 8, 8, false, null, null, null);
		setBounds(37920, xPosition, 7, 75, item_widget);
		xPosition += 39;

		addToItemGroup(37921, 1, 60, 8, 8, false, null, null, null);
		setBounds(37921, xPosition, 7, 76, item_widget);
		xPosition += 39;

		addToItemGroup(37922, 1, 60, 8, 8, false, null, null, null);
	...
	public static void bossTeles(TextDrawingArea[] tda) {
		...
		addText(36806, "Bosses", tda, 2, 0xFFA500, true, true);
		addText(36807, "Possible drops and rewards:", tda, 0, 0xFFA500, false, true);
		addText(36808, "", tda, 1, 0xFFA500, true, true);
		addText(36809, "Information:", tda, 1, 0xFFA500, false, true);
		addNpc(36810, 50);
		interfaceCache[36810].modelZoom = 2400;
		addHoverButton(36811, dir, 5, 99, 27, "Teleport", -1, 36812, 1);
		addHoveredButton(36812, dir, 6, 99, 27, 36813);
		addText(36814, "Teleport", tda, 1, 0xffffff, true, true);
		int x = 5, y = 5;
		tab.totalChildren(15);
		tab.child(0, 36801, x, 9 + y);
		tab.child(1, 36802, 472 + x, 12 + y);
		tab.child(2, 36803, 472 + x, 12 + y);
		tab.child(3, 36805, 248 + x, 12 + y);
		tab.child(4, 36830, 9 + x, 57 + y);
		tab.child(5, 36806, 68 + x, 40 + y);
		tab.child(6, 36920, 145 + x, 254 + y);
		tab.child(7, 36807, 168 + x, 240 + y);
		tab.child(8, 36808, 309 + x, 44 + y);
		tab.child(9, 36809, 334 + x, 81 + y);
		tab.child(10, 36925, 312 + x, 98 + y);// what id do i have to use
		tab.child(11, 36810, 150 + x, 80 + y);
		tab.child(12, 36811, 174 + x, 205 + y);
		tab.child(13, 36812, 174 + x, 205 + y);
		tab.child(14, 36814, 224 + x, 211 + y);
		...
		for (int i = 0; i < 8; i++) {
			String[] drops = { "Test1", "Test2", "Test3", "Test4", "Test5", "Test6", "Test7", "Test8" };
			addText(36931 + i, i == 0 ? "Notable Drops:" : drops[i], tda, 0, i == 0 ? 0xFFA500 : 0xff9040, false, true);
		...
		RSInterface.interfaceCache[36810].npcDisplay = 5886;

		RSInterface bosses = addInterface(36830);
		int yy = 0;
		bosses.totalChildren(55);
		for (int i = 0; i < 25; i++) {
			String[] names = { "Abby Sire", "Bandos Avatar","Barrelchest", "Bork", "Callisto","Cerberus","Chaos Elemental","Corporeal Beast","Dagannoth Kings", "Giant Mole","Giant Roc", "Glacor", "Godwars", "Kalphite Queen",
					"King Black Dragon", "Kraken", "Lizardman Shaman", "Mutant", "Nex",
					"Phoenix", "Scorpia","Skotizo", "Slash Bash", "Tormented Demons", "Venenatis"};
			addHoverButton(36831 + i, dir, 3, 115, 19, "View", -1, 36850 + i, 1);
			addHoveredButton(36860 + i, dir, 4, 115, 19, 36829);
			addText(36890 + i, names[i], tda, 0, 0xff9040, true, true);

			bosses.child(i, 36831 + i, 0, yy);
			bosses.child(i + 5, 36860 + i, 0, yy);
			bosses.child(i + 30, 36890 + i, 54, yy + 4);
			yy += 19;
		}
		bosses.width = 105;
		bosses.height = 256;
...
/*	public static void bossTeles(TextDrawingArea[] tda) {
		...
		items.scrollMax = 200;
		RSInterface.interfaceCache[36800].npcDisplay = 5886;

		RSInterface bosses = addInterface(36830);
		int yy = 0;
		bosses.totalChildren(55);
		for (int i = 0; i < 25; i++) {
			String[] names = { "Abby Sire", "Bandos Avatar","Barrelchest", "Bork", "Callisto","Cerberus","Chaos Elemental","Corporeal Beast","Dagannoth Kings", "Giant Mole", "Glacor", "Godwars", "Kalphite Queen",
					"King Black Dragon", "Kraken", "Lizardman Shaman", "Mutant", "Nex",
					"Phoneix", "Scorpia","Skotizo", "Slash Bash", "Tormented Demons", "Tornado", "Venenatis"};
			addHoverButton(36831 + i, dir, 3, 115, 19, "View", -1, 36860 + i, 1);
			addHoveredButton(36860 + i, dir, 4, 115, 19, 36829);
			addText(36890 + i, names[i], tda, 0, 0xff9040, true, true);

			bosses.child(i, 36831 + i, 0, yy);
			bosses.child(i + 25, 36860 + i, 0, yy);
			bosses.child(i + 30, 36890 + i, 54, yy + 4);
			yy += 19;
		}
		bosses.width = 105;
	...
	public static void newAchievements(TextDrawingArea[] tda) {
		RSInterface rsi = addInterface(53200);
		int childId = 53201;
		addSprite(childId++, 0, "data/curs2/NewAchievements/BG");

		// Left Scroll
		RSInterface leftScroll = addInterface(childId++);
		int scrollChildId = 53400;
		leftScroll.width = 324;
		leftScroll.height = 277;
		leftScroll.scrollMax = 4000;
		leftScroll.totalChildren(150 * 5);
		int scrollFrame = 0;
		int yOffset = 0;
		int yOffset2 = 15;
		int yOffset3 = 13;
		for (int i = 0; i < 150; i++) {
			addSprite(scrollChildId, 0, "Interfaces/NewAchievements/info");
			leftScroll.child(scrollFrame++, scrollChildId++, 0, 0 + yOffset);
			yOffset += 57;
			
			addText(scrollChildId, "", tda, 2, 0xFF981F, true, true);
			leftScroll.child(scrollFrame++, scrollChildId++, 180, 0 + yOffset2 - 2);
			addText(scrollChildId, "", tda, 2, 0xFF981F, true, true);
			leftScroll.child(scrollFrame++, scrollChildId++, 180, 0 + yOffset2 + 12);
			addText(scrollChildId, "", tda, 0, 0xFF981F, false, true);
			leftScroll.child(scrollFrame++, scrollChildId++, 222, 0 + yOffset2 + 28);
			yOffset2 += 57;
			
			addToItemGroup(scrollChildId, 1, 1, 0, 0, false, "", "", "");
			leftScroll.child(scrollFrame++, scrollChildId++, 13, 0 + yOffset3);
			yOffset3 += 57;
		}

		// Right Scroll
		RSInterface rightScroll = addInterface(childId++);
		int scrollChildId2 = 52950;
		rightScroll.width = 99;
		rightScroll.height = 93;
		rightScroll.scrollMax = 320;
		rightScroll.totalChildren(1);
		int scrollFrame2 = 0;

		addToItemGroup(scrollChildId2, 3, 10, 0, 0, false, "", "", "");
		rightScroll.child(scrollFrame2++, scrollChildId2++, 0, 0);

		addText(childId++, "Polaris Achievements", tda, 2, 0xFF981F, true, true);

		String[] tabs = new String[] { "PvM", "PvP", "Skill" };
		for (int i = 0; i < 3; i++) {
			addHoverButton(childId++, "Interfaces/NewAchievements/style", 0, 42, 24, tabs[i], -1, childId, 1);
			addHoveredButton(childId++, "Interfaces/NewAchievements/style", 1, 42, 24, childId++);
			addText(childId++, tabs[i], tda, 0, 0xFF981F, true, true);
		}

		String[] tiers = new String[] { "Easy", "Medium", "Hard", "Elite" };
		for (int i = 0; i < 4; i++) {
			addHoverButton(childId++, "Interfaces/NewAchievements/tab", 0, 130, 27, tiers[i], -1, childId, 1);
			addHoveredButton(childId++, "Interfaces/NewAchievements/tab", 1, 130, 27, childId++);
			addText(childId++, tiers[i], tda, 2, 0xFF981F, true, true);
		}

		addHoverButton(childId++, "Interfaces/NewAchievements/CLAIM", 0, 90, 17, "Claim", -1, childId, 1);
		addHoveredButton(childId++, "Interfaces/NewAchievements/CLAIM", 1, 90, 17, childId++);
		addText(childId++, "Claim", tda, 2, 0xFF981F, true, true);
		
		addHoverButton(childId++, "Interfaces/Achievements/IMAGE", 15, 16, 16, "Close Window", -1, childId, 3);
		addHoveredButton(childId++, "Interfaces/Achievements/IMAGE", 16, 16, 16, childId++);

		rsi.totalChildren(childId - 53201 - 9);
		int frame = 0;
		childId = 53201;
		rsi.child(frame++, childId++, 10, 10);
		rsi.child(frame++, childId++, 151, 44);
		rsi.child(frame++, childId++, 24, 217);

		rsi.child(frame++, childId++, 256, 20);

		int startX = 16;
		for (int i = 0; i < 3; i++) {
			rsi.child(frame++, childId++, startX, 47);
			rsi.child(frame++, childId++, startX, 47);
			childId++;
			rsi.child(frame++, childId++, startX + 20, 54);
			startX += 43;
		}

		int startY = 72;
		for (int i = 0; i < 4; i++) {
			rsi.child(frame++, childId++, 15, startY);
			rsi.child(frame++, childId++, 15, startY);
			childId++;
			rsi.child(frame++, childId++, 15 + 65, startY + 6);
			startY += 27;
		}
		
		rsi.child(frame++, childId++, 35, 189);
		rsi.child(frame++, childId++, 35, 189);
		childId++;
		rsi.child(frame++, childId++, 80, 190);
		
		rsi.child(frame++, childId++, 475, 18);
		rsi.child(frame++, childId++, 475, 18);
		childId++;
	}
	...
	public static void playerOwnedShopInterface(TextDrawingArea[] tda) {
		...
		// String dir = "data/Interfaces/Pos/ok";
		addSpriteLoader(32601, 1056);

		addHoverButtonWSpriteLoader(32602, 1061, 115, 22, "Search Players", 0, 32603, 1);
		addHoveredImageWSpriteLoader(32603, 1062, 115, 22, 32604);

		addHoverButtonWSpriteLoader(32616, 1061, 115, 22, "Search Items", 0, 32617, 1);
		addHoveredImageWSpriteLoader(32617, 1062, 115, 22, 32618);

		addHoverButtonWSpriteLoader(32606, 1015, 16, 16, "Close Window", 250, 32607, 3);
		addHoveredImageWSpriteLoader(32607, 1016, 16, 16, 32608);
		addText(32610, "", tda, 2, 0xff7000, true, true);
		addText(32611, "", tda, 1, 0xff7000, false, true);

		addHover(54098, 1, 0, 54099, 0, "", 0, 0, "");
		addHovered(54099, 1, "", 0, 0, 54100);
		addText(54101, "", tda, 1, 0xff7000, false, true);

		addText(54105, "Search for Items", tda, 1, 0xff7000, false, true);
		addText(54106, "Search for Players", tda, 1, 0xff7000, false, true);

		addText(54109, "Claim", tda, 0, 0xff7000, true, true);

		addText(54111, "Edit your shop", tda, 1, 0xff7000, true, true);

		addHoverButtonWSpriteLoader(32613, 1059, 99, 19, "Claim earnings", 0, 32614, 1);
		addHoveredImageWSpriteLoader(32614, 1060, 99, 19, 32615);

		addHoverButtonWSpriteLoader(34619, 1057, 99, 32, "Edit your shop", 0, 34620, 1);
		addHoveredImageWSpriteLoader(34620, 1058, 99, 32, 34621);

		int x = 10, y = 10;
		tab.totalChildren(21);
		tab.child(0, 32601, 6, 10);
		tab.child(1, 32602, 166 + x, 240 + y);
		tab.child(4, 32616, 38 + x, 240 + y);
		tab.child(11, 32603, 166 + x, 240 + y);
		tab.child(20, 32617, 38 + x, 240 + y);
		tab.child(2, 32606, 463 + x, 4 + y);
		tab.child(3, 32607, 463 + x, 4 + y);
		// tab.child(4, 32611, 90 + x, 276 + y);
		tab.child(5, 32620, 9 + x, 27 + y);
		tab.child(6, 32622, 350 + x, 27 + y);
		tab.child(7, 32610, 242 + x, 3 + y);
		tab.child(8, 54098, 382, 272 + y);
		tab.child(9, 54099, 382, 272 + y);
		tab.child(10, 54101, 394, 275 + y);

		tab.child(12, 54105, 50 + x, 217 + y);
		tab.child(13, 54106, 172 + x, 217 + y);

		tab.child(14, 32613, 338 + x, 245 + y);
		tab.child(15, 32614, 338 + x, 245 + y);

		tab.child(16, 34619, 338 + x, 208 + y);
		tab.child(17, 34620, 338 + x, 208 + y);

		tab.child(18, 54109, 388 + x, 248 + y);
		tab.child(19, 54111, 385 + x, 215 + y);

		tab2.width = 350;
		tab2.height = 163;
		tab2.scrollMax = 120;
		itemGroup(32621, 8, 4, 10, 10);
		tab2.totalChildren(1);
		tab2.child(0, 32621, 14, 6);

		for (int i = 0; i < 100; i++) {
			addClickableText(32623 + i, "  " + i, "View", tda, 0, 0xeb981f, false, true, 632);
		}
		tab3.width = 88;
		tab3.height = 163;
		tab3.scrollMax = 10 + (75 * 14);
	...
	public static void newStarter(TextDrawingArea[] tda) {
		...
		addText(52752, "CelestialX Account Setup", 0xff9933, true, true, 52, tda, 3);
		...
		addText(52764, "Play CelestialX as an Iron man.", 0xff9933, true, true, 52, tda, 0);
	...
	public static void TeleTAB1() {
		...
		addText(50430, "CelestialX Teleports", fonts, 2, 0xff9040, true, true); // rename it whatever
	...
	public static void TeleTAB2() {
		...
		addText(50430, "CelestialX Teleports", fonts, 2, 0xff9040, true, true);
	...
	public static void playersOnline(TextDrawingArea[] tda) {
		...
		addText(57007, "CelestialX.org", 0xff9933, true, true, -1, tda, 0);
	...
	public static void shortCutInterface() {
		...
		RSInterface.setBounds(17931, 13, 6, 0, scInterface);
		RSInterface.setBounds(17932, 48, 39, 1, scInterface);
		RSInterface.setBounds(17933, 194, 39, 2, scInterface);
		RSInterface.setBounds(17934, 343, 39, 3, scInterface);
		RSInterface.setBounds(17935, 48, 107, 4, scInterface);
		RSInterface.setBounds(17936, 194, 107, 5, scInterface);
		RSInterface.setBounds(17937, 343, 107, 6, scInterface);
		RSInterface.setBounds(17938, 48, 181, 7, scInterface);
		RSInterface.setBounds(17939, 194, 181, 8, scInterface);
		RSInterface.setBounds(17940, 343, 181, 9, scInterface);
		RSInterface.setBounds(17941, 48, 248, 10, scInterface);
		RSInterface.setBounds(17942, 194, 248, 11, scInterface);
		RSInterface.setBounds(17943, 343, 248, 12, scInterface);
		RSInterface.setBounds(17944, 477, 8, 13, scInterface);
	}

	public static void optionsInterface() {
		String[] options = new String[] { "525 Frame", "Old Hits", "10x Hits", "HD Shading" };
		String[] options2 = new String[] { "Cursors", "Smilies", "Censor", "Mipmapping" };
	...
	public static void unpack(CacheArchive streamLoader, TextDrawingArea textDrawingAreas[],
		...
		while (stream.currentOffset < stream.buffer.length) {
			...
			if (rsInterface.type == 4) {
				rsInterface.message = stream.readString().replaceAll("RuneScape", "CelestialX");
				if (rsInterface.message.toLowerCase().equals("the bank of CelestialX")) {
					rsInterface.message = "        The Bank of CelestialX";
				}
		...
		questTabInterface();
		questTabInterface2();
		MBoxInterface(textDrawingAreas);
		newAchievements(textDrawingAreas);
	...
	public static void achievementsInterface() {
		...
		int k = 0;
		int y = 25;
		for (int i = 45005; i < 45116; i++) {
			scroll.child(k, i, 6, y);
			y += 16;
			k++;
			if (i == 45035 || i == 45068 || i == 45102 || i >= 45112) {
	...
	private static void questTabInterfaceNew() {
		...
		addText(26609, "www.CelestialX.org", fonts, 0, 16750623, false, true);
	...
	public static void statistics1(final TextDrawingArea[] tda) {
		...
		addText(id + 3, "CelestialX", 0xAF6A1B, false, true, 0, tda, 2);
	...
	public static void collectBuy(TextDrawingArea[] TDA) {
		RSInterface rsinterface = addTabInterface(58700);
		int x = 9;
		addHDSprite(58701, 642, 642);
		addHoverButton(58702, CLOSE_BUTTON, CLOSE_BUTTON, 16, 16, "Close", 0, 58703, 1);
		addHoveredButton(58703, CLOSE_BUTTON_HOVER, CLOSE_BUTTON_HOVER, 16, 16, 58704);
		addHoverButton(58758, 607, 607, 29, 23, "Back", 0, 58759, 1);
		addHoveredButton(58759, 608, 608, 29, 23, 58760);
		addText(58769, "Choose an item to exchange", TDA, 0, 0x96731A, false, true);
		addText(58770, "Select an item from your invertory to sell.", TDA, 0, 0x958E60, false, true);
		addText(58771, "0", TDA, 0, 0xB58338, true, true);
		addText(58772, "1 gp", TDA, 0, 0xB58338, true, true);
		addText(58773, "0 gp", TDA, 0, 0xB58338, true, true);
		addHoverButton(58793, 653, 653, 40, 36, "[GE]", 0, 58794, 1);
		addHoveredButton(58794, 652, 652, 40, 36, 58795);
		addHoverButton(58796, 653, 653, 40, 36, "[GE]", 0, 58797, 1);
		addHoveredButton(58797, 652, 652, 40, 36, 58798);
		addItemOnInterface(58780, 2901, new String[] { null });
		addItemOnInterface(58781, 2901, new String[] { "Collect" });
		addItemOnInterface(58782, 2902, new String[] { "Collect" });
		addText(58784, "", TDA, 0, 0xFFFF00, false, true);
		addText(58785, "", TDA, 0, 0xFFFF00, false, true);
		addText(58787, "N/A", TDA, 0, 0xB58338, false, true);
		addText(58788, "", TDA, 0, 0xFFFF00, true, true);
		addText(58789, "", TDA, 0, 0xFFFF00, true, true);
		addHoverButton(58800, 640, 640, 20, 20, "Abort offer", 0, 58801, 1);
		addHoveredButton(58801, 641, 641, 20, 20, 58802);
		rsinterface.totalChildren(24);
		rsinterface.child(0, 58701, 4 + x, 23);// 385, 260
		rsinterface.child(1, 58702, 464 + x, 33);// 435, 260
		rsinterface.child(2, 58703, 464 + x, 33);
		rsinterface.child(3, 58758, 19 + x, 284);
		rsinterface.child(4, 58759, 19 + x, 284);
		rsinterface.child(5, 58769, 202 + x, 71);
		rsinterface.child(6, 58770, 202 + x, 98);
		rsinterface.child(7, 58771, 142 + x, 185);
		rsinterface.child(8, 58772, 354 + x, 185);
		rsinterface.child(9, 58773, 252 + x, 246);
		rsinterface.child(10, 58793, 386 + x, 256 + 23);
		rsinterface.child(11, 58794, 386 + x, 256 + 23);
		rsinterface.child(12, 58796, 435 + x, 256 + 23);
		rsinterface.child(13, 58797, 435 + x, 256 + 23);
		rsinterface.child(14, 58780, 97 + x, 97);
		rsinterface.child(15, 58781, 385 + 4 + x, 260 + 23);
		rsinterface.child(16, 58782, 435 + 4 + x, 260 + 23);
		rsinterface.child(17, 58784, 385 + 4 + x, 260 + 23);
		rsinterface.child(18, 58785, 435 + 4 + x, 260 + 23);
		rsinterface.child(19, 58787, 108, 136);
		rsinterface.child(20, 58788, 214 + x, 249 + 23);
		rsinterface.child(21, 58789, 214 + x, 263 + 23);
		rsinterface.child(22, 58800, 345 + x, 250 + 23);
		rsinterface.child(23, 58801, 345 + x, 250 + 23);
	}
/*	public static void collectBuy(TextDrawingArea[] TDA) {
		RSInterface rsinterface = addTabInterface(57700);
		int x = 9;
	...
	public static void beastOfBurden(TextDrawingArea[] wid) {
		RSInterface familiarInventory = addTabInterface(24000).totalChildrenReturn(4).childReturn(0, 24001, 104 - 25,
				16);
		RSInterface.addHDSprite(24001, 675, 675);
		familiarInventory.childReturn(1, 24002, 95, 59);
		RSInterface.addInventoryItemGroup(24002, 5, 6);
		familiarInventory.childReturn(2, 24003, 216 - 25, 25);
		RSInterface.addText(24003, "Familiar's Inventory", wid, 2, 0xFF8C00, false, true);
		familiarInventory.childReturn(3, 24004, 445 - 25, 27);
		RSInterface.addButton(24004, CLOSE_BUTTON, CLOSE_BUTTON, 16, 16, "Close", 5);
	}
	...
	public static void beastOfBurden2(TextDrawingArea[] wid) {
		...
		RSInterface.addInventoryItemGroup2(24006, 7, 4);
	}
	...
	public static void newTele(TextDrawingArea[] tda) {
		RSInterface tab = addInterface(61200);
		addSpriteLoader(61201, 1010);

		addHoverButtonWSpriteLoader(61202, 1015, 17, 17, "Close Window", 0, 61203, 1);
		addHoveredImageWSpriteLoader(61203, 1016, 17, 17, 61204);
		...
		addText(60659, "Dungeons", tda, 1, 0xF7FE2E, true, true);
		addText(60660, "Wilderness", tda, 1, 0xF7FE2E, true, true);

		int x = 11, y = 12;
		tab.totalChildren(19);
		int child = 0;
		tab.child(child++, 61201, x, y);
		tab.child(child++, 61202, x + 464, y + 10);
		tab.child(child++, 61203, x + 464, y + 10);

		tab.child(child++, 61248, x + 137, y + 69);

		int r = 6;
		int z = 40;
		tab.child(child++, 60602, 19 + r, 41 + z); // Tab 1 Hover
		tab.child(child++, 60603, 19 + r, 41 + z); // Tab 1
		z += 5;
		tab.child(child++, 60605, 19 + r, 76 + z); // Tab 2 Hover
		tab.child(child++, 60606, 19 + r, 76 + z); // Tab 2
		z += 5;
		tab.child(child++, 60608, 19 + r, 111 + z); // Tab 3 Hover
		tab.child(child++, 60609, 19 + r, 111 + z); // Tab 3
		z += 5;
		tab.child(child++, 60611, 19 + r, 146 + z); // Tab 4 Hover
		tab.child(child++, 60612, 19 + r, 146 + z); // Tab 4
		z += 5;
		tab.child(child++, 60614, 19 + r, 181 + z); // Tab 5 Hover
		tab.child(child++, 60615, 19 + r, 181 + z); // Tab 5
		z = 40;
		tab.child(child++, 60656, 75 + r, 50 + z); // Title 1
		z += 5;
		tab.child(child++, 60657, 75 + r, 85 + z); // Title 2
		z += 5;
		tab.child(child++, 60658, 75 + r, 120 + z); // Title 3
		z += 5;
		tab.child(child++, 60659, 75 + r, 155 + z); // Title 4
		z += 5;
		tab.child(child++, 60660, 75 + r, 190 + z); // Title 5

		RSInterface normal = addInterface(61248);
		normal.width = 324;
		normal.height = 227;
		normal.scrollMax = 1050;

		normal.totalChildren(51);
		addSpriteLoader(61249, 1031);

		int childId = 0;
		normal.child(childId++, 61249, 0, 0);

		int yz = 7;
		for (int i = 0; i < 50; i++) {
			addClickableText(61250 + i, "", "Select", fonts, 1, 0xFF8900, 310, 24, true);
			normal.child(childId, 61250 + i, 5, yz);
			childId++;
			yz += 30;
		}
	...
	public static void addClickableText(int id, String text, String tooltip, TextDrawingArea tda[], int idx, int color,
			int width, int height, boolean shadow) {
		RSInterface Tab = addTabInterface(id);
		Tab.parentID = id;
		Tab.id = id;
		Tab.type = 4;
	...
	private static Sprite getSprite(int spriteId, CacheArchive streamLoader, String spriteName) {
		if (spriteName.equals("headicons") && spriteId == 0)
			return null;
		long l = (TextClass.method585(spriteName) << 8) + spriteId;
		Sprite sprite = (Sprite) spriteCache.get(l);
		if (sprite != null)
			return sprite;
		try {
	...
	public enum SummonData {
		SPIRIT_WOLF(1, new int[] { GOLD_CHARM, 2859 }, 7), SPIRIT_DREADFOWL(2, new int[] { GOLD_CHARM, 2138 },
				8), SPIRIT_SPIDER(3, new int[] { GOLD_CHARM, 6291 }, 8), THORNY_SNAIL(4, new int[] { GOLD_CHARM, 3363 },
						9), GRANITE_CRAB(5, new int[] { GOLD_CHARM, 440 }, 7), SPIRIT_MOSQUITO(6,
								new int[] { GOLD_CHARM, 6319 }, 1), DESERT_WYRM(7, new int[] { GREEN_CHARM, 1783 },
										45), SPIRIT_SCORPION(8, new int[] { CRIMSON_CHARM, 3095 }, 57), SPIRIT_TZ_KIH(9,
												new int[] { CRIMSON_CHARM, OBSIDIAN_CHARM }, 64), ALBINO_RAT(10,
														new int[] { BLUE_CHARM, 2134 }, 75), SPIRIT_KALPHITE(11,
																new int[] { BLUE_CHARM, 3138 }, 51), COMPOST_MOUND(12,
																		new int[] { GREEN_CHARM, 6032 },
																		47), GIANT_CHINCHOMPA(13,
																				new int[] { BLUE_CHARM, 9976 },
																				84), VAMPIRE_BAT(14,
																						new int[] { CRIMSON_CHARM,
																								3325 },
																						81), HONEY_BADGER(15, 92040,
																								12065, 12433,
																								new int[] {
																										CRIMSON_CHARM,
																										12156 },
	...
	private static void equipmentScreenInterface() {
		...
		for (int i = 1675; i <= 1684; i++) {
			RSInterface rsi = interfaceCache[i];
			RSInterface.textColor(i, 0xFF9200);
			rsi.centerText = false;
		}
		for (int i = 1686; i <= 1687; i++) {
			RSInterface rsi = interfaceCache[i];
	...
	public static void addSummoningText(int i, String s, int k, boolean l, boolean m, int a, TextDrawingArea[] TDA,
		...
		RSInterface.message = s;
		RSInterface.message = s;
		com.client.client.RSInterface.textColor(i, k);
		RSInterface.interfaceShown = true;
		RSInterface.hoverType = -1;
	}
