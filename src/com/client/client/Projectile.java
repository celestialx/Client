...
final class Projectile extends Animable {
	...
	public void calculateTracking(int currentTime, int targetY, int targetZ, int targetX)
	{
		if(!isMoving)
		{
			...
			currentPositionX = startX + (distanceX * radius) / distanceOverall;
			currentPositionY = startY + (distanceY * radius) / distanceOverall;
			currentPositionZ = drawHeight;
		}
		double d1 = (speed + 1) - currentTime;
		speedX = (targetX - currentPositionX) / d1;
		speedY = (targetY - currentPositionY) / d1;
		speedOverall = Math.sqrt(speedX * speedX + speedY * speedY);
		if(!isMoving)
			speedZ = -speedOverall * Math.tan(slopeHeight * 0.02454369D);
		timeLeftTillFinishZ = (2D * (targetZ - currentPositionZ - speedZ * d1)) / (d1 * d1);
	}
	...
	{
		isMoving = true;
		currentPositionX += speedX * nextFrame;
		currentPositionY += speedY * nextFrame;
		currentPositionZ += speedZ * nextFrame + 0.5D * timeLeftTillFinishZ * nextFrame * nextFrame;
		speedZ += timeLeftTillFinishZ * nextFrame;
