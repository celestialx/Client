...
public class RSApplet extends Applet implements Runnable, MouseListener, MouseMotionListener, MouseWheelListener,
	...
	public void rebuildFrame(boolean undecorated, int width, int height, boolean resizable, boolean full) {
		...
		if (!webc) {
			/*
			 * JFrame frame = Jframe.frame; frame.setPreferredSize(new Dimension(width,
			 * height)); frame.setResizable(resizable); frame.setUndecorated(undecorated);
			 */
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		...
		if (mouseX > 0 && mouseX < 512 && mouseY > Client.getClient().clientHeight - 165
			...
			if (zoom && Client.openInterfaceID == -1) {
				Client.clientZoom += rotation * 25;

				if (Client.clientZoom < -200 && Client.clientSize > 0)
					Client.clientZoom = -200;
				else if (Client.clientZoom < -500 && Client.clientSize == 0)
					Client.clientZoom = -500;
				if (Client.clientZoom > 1300 && Client.clientSize == 0)
					Client.clientZoom = 1300;
				else if (Client.clientZoom > 2000 && Client.clientSize > 0)
					Client.clientZoom = 2000;
			}
	...
	public boolean handleInterfaceScrolling(MouseWheelEvent event) {
		...
		if (tabInterfaceID != -1) {
			...
			offsetX = Client.getClient().clientSize == 0 ? Client.getClient().clientWidth - 218
					: (Client.getClient().clientSize == 0 ? 28 : Client.getClient().clientWidth - 197);
			offsetY = Client.getClient().clientSize == 0 ? Client.getClient().clientHeight - 298
					: (Client.getClient().clientSize == 0 ? 37
							: Client.getClient().clientHeight - (Client.getClient().clientWidth >= 900 ? 37 : 74)
									- 267);
