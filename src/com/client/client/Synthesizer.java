...
final class Synthesizer {
	...
	public int[] synthesize(int sampleLength, int j) {
		...
		if (j < 10)
			return samples;
		double rate = sampleLength / (j + 0.0D);
		samp1.resetValues();
		samp2.resetValues();
		int l = 0;
		int i1 = 0;
		...
		if (samp3 != null) {
			samp3.resetValues();
			samp4.resetValues();
			l = (int) (((samp3.anInt539 - samp3.anInt538) * 32.768000000000001D) / rate);
			i1 = (int) ((samp3.anInt538 * 32.768000000000001D) / rate);
		}
		int k1 = 0;
		int l1 = 0;
		int i2 = 0;
		if (samp5 != null) {
			samp5.resetValues();
			samp6.resetValues();
			k1 = (int) (((samp5.anInt539 - samp5.anInt538) * 32.768000000000001D) / rate);
			l1 = (int) ((samp5.anInt538 * 32.768000000000001D) / rate);
		}
		for (int j2 = 0; j2 < 5; j2++)
			if (anIntArray106[j2] != 0) {
				phase[j2] = 0;
				anIntArray119[j2] = (int) (anIntArray108[j2] * rate);
				anIntArray120[j2] = (anIntArray106[j2] << 14) / 100;
				anIntArray121[j2] = (int) (((samp1.anInt539 - samp1.anInt538) * 32.768000000000001D * Math.pow(1.0057929410678534D, cents[j2])) / rate);
				anIntArray122[j2] = (int) ((samp1.anInt538 * 32.768000000000001D) / rate);
			}
		...
		if (filter.anIntArray665[0] > 0 || filter.anIntArray665[1] > 0) {
			...
			int i5 = filter.method544(0, k3 / 65536F);
			int i6 = filter.method544(1, k3 / 65536F);
			if (sampleLength >= i5 + i6) {
				...
				do {
					...
					if (j7 >= sampleLength - i5)
						break;
					i5 = filter.method544(0, k3 / 65536F);
					i6 = filter.method544(1, k3 / 65536F);
					l7 += c;
				} while (true);
